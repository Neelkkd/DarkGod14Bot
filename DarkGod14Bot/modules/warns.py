...
import telegram
from DarkGod14Bot import WHITELIST_USERS, dispatcher
...
from DarkGod14Bot.modules.helper_funcs.extraction import (
    extract_text,
    extract_user,
...
from DarkGod14Bot.modules.helper_funcs.string_handling import split_quotes
from DarkGod14Bot.modules.log_channel import loggable
from DarkGod14Bot.modules.sql import warns_sql as sql
...
def warn(
    ...
    if num_warns >= limit:
        ...
        # message.bot.send_sticker(chat.id, BAN_STICKER)  # DarkGod14Bot's sticker
        keyboard = None
        log_reason = (
    ...
    if args:
        if args[0].lower() in ("on", "yes"):
            ...
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
                f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
                f"Has disabled strong Kicks. I will use normal punch on users."
            )
