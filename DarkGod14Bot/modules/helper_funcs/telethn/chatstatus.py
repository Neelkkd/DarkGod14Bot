from DarkGod14Bot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from DarkGod14Bot import SUDO_USERS
...
async def DarkGod14Bot_is_admin(chat_id: int):
    status = False
    DarkGod14Bot = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if DarkGod14Bot.id == user.id:
            status = True
            break
    return status
