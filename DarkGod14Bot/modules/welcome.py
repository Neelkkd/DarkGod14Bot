...
import DarkGod14Bot.modules.sql.welcome_sql as sql
import DarkGod14Bot
from DarkGod14Bot import (
    DEV_USERS,
    LOGGER,
    OWNER_ID,
    SUDO_USERS,
...
from DarkGod14Bot.modules.helper_funcs.chat_status import (
    is_user_ban_protected,
    user_admin,
)
from DarkGod14Bot.modules.helper_funcs.misc import build_keyboard, delete, revert_buttons
from DarkGod14Bot.modules.helper_funcs.msg_types import get_welcome_type
from DarkGod14Bot.modules.helper_funcs.string_handling import (
    escape_invalid_curly_brackets,
    markdown_parser,
)
from DarkGod14Bot.modules.log_channel import loggable
from DarkGod14Bot.modules.sql.clear_cmd_sql import get_clearcmd
from DarkGod14Bot.modules.sql.global_bans_sql import is_user_gbanned
...
def new_member(update: Update, context: CallbackContext):
    ...
    for new_mem in new_members:
        ...
        if new_mem.id == bot.id and not DarkGod14Bot.ALLOW_CHATS:
        ...
        if user.id == new_mem.id:
            if should_mute:
                ...
                if welc_mutes == "captcha":
                    ...
                    message = msg.reply_photo(fileobj, caption=f'Welcome [{escape_markdown(new_mem.first_name)}](tg://user?id={user.id}). Click the correct button to get unmuted! You have 60 seconds before you get kicked',
...
def welcomemute(update: Update, context: CallbackContext) -> str:
    ...
    if len(args) >= 1:
        if args[0].lower() in ("off", "no"):
            ...
            msg.reply_text(
                "I will now mute people when they join until they prove they're not a bot.\nThey have to solve a captcha within 60 seconds to get unmuted."
            )
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
                f"#WELCOME_MUTE\n"
...
WELC_MUTE_HELP_TXT = (
    ...
    "*Note:* Strong/Captcha mode kicks a user from the chat if they dont verify in 60 seconds. They can always rejoin though"
)
...
DEFENSE_HANDLER = CommandHandler("setdefense", setDefense, filters=Filters.chat_type.groups)
GETDEF_HANDLER = CommandHandler("defense", getDefense, filters=Filters.chat_type.groups)
